...
from ..core.patch_advisor import PatchAdvisor
...
@router.post("/api/hosts", response_model=HostResponse)
async def create_host(host: HostCreate, session: AsyncSession = Depends(get_db)):
    """Create a new host"""
...
@router.get("/api/hosts", response_model=List[HostResponse])
async def get_hosts(session: AsyncSession = Depends(get_db)):
    """Get all hosts"""
...
async def delete_host(host_id: int, session: AsyncSession = Depends(get_db)):
    """Delete a host"""
...
@router.post("/api/scan/{host_id}")
async def scan_host(
    host_id: int,
    fast_mode: bool = Query(True),
    categories: str = Query("all"),  # Comma-separated categories
    filter_patched: bool = Query(True),  # 패치된 CVE 제외
    filter_old_cve: bool = Query(True),  # 오래된 CVE 제외
    filter_other_os: bool = Query(True),  # 타 OS 관련 CVE 제외
    ...
    # 이제 findings는 scan_id로 분리되므로 삭제하지 않음
    # 각 스캔은 독립적인 findings를 가짐
    ...
    # Parse categories
    selected_categories = [cat.strip() for cat in categories.split(",")] if categories else ["all"]

    # 스캔 옵션
    scan_options = {
        "filter_patched": filter_patched,
        "filter_old_cve": filter_old_cve,
        "filter_other_os": filter_other_os
    }

    asyncio.create_task(
        _perform_scan(host_id, scan_history.id, fast_mode, selected_categories, scan_options)
    )
...
async def _perform_scan(host_id: int, scan_id: int, fast_mode: bool = True, selected_categories: List[str] = None, scan_options: dict = None):
    ...
    async with async_session_maker() as session:
        try:
            ...
            packages = await scanner.scan_local_packages(host.os_type)

            filtered_packages = scanner.filter_critical_packages(packages, selected_categories)

            os_info = await scanner.detect_os_version()
            host.os_version = os_info["os_version"]

            nvd_client = NVDClient()
            matcher = VulnerabilityMatcher(nvd_client)

            # 스캔 옵션 전달
            matcher.set_scan_options(scan_options)
            ...
            scan_record = scan.scalar_one()

            scan_record.scan_completed = datetime.utcnow()
            scan_record.status = "completed"
            scan_record.packages_found = len(filtered_packages)
            scan_record.cves_found = stats["total_cves"]
            scan_record.high_risk_count = stats["high_risk_count"]
...
async def scan_local_system(
    categories: str = Query("all"),
    filter_patched: bool = Query(True),
    filter_old: bool = Query(True),
    session: AsyncSession = Depends(get_db)
):
    """
    로컬 시스템 빠른 스캔
    현재 시스템의 패키지와 취약점을 스캔합니다.
    """
    # 로컬 호스트 찾기 또는 생성
    ...
    packages = await scanner.scan_local_packages(os_type)
    
    vulnerabilities = []
    total_packages = len(packages)
    
    category_list = categories.split(',') if categories != 'all' else ['all']
    
    # 매처를 사용하여 취약점 조회
    nvd = NVDClient()
    matcher = VulnerabilityMatcher(nvd)
    
    # 패키지 이름 목록 추출
    pkg_names = list(set(pkg['name'] for pkg in packages))
    
    # 배치로 CVE 조회 (최적화)
    for pkg in packages[:50]:  # 처음 50개만 빠른 스캔
        # 카테고리 필터링
        if 'all' not in category_list:
            pkg_category = _get_package_category(pkg['name'])
            if pkg_category not in category_list:
                continue
        
        try:
            # CVE 조회 (캐시 활용)
            cves = await nvd.search_cves_for_package(pkg['name'], pkg.get('version'))
            
            for cve in cves[:5]:  # 패키지당 최대 5개 CVE
                # 필터링
                if filter_patched and cve.get('patched'):
                    continue
                if filter_old:
...
def _get_package_category(pkg_name: str) -> str:
    """패키지 카테고리 추정"""
    security_pkgs = ['openssl', 'gnupg', 'openssh', 'libcrypto', 'libssl', 'pam']
    system_pkgs = ['libc', 'systemd', 'kernel', 'glibc', 'bash', 'coreutils']
    
    name_lower = pkg_name.lower()
    for s in security_pkgs:
        if s in name_lower:
            return 'security'
    for s in system_pkgs:
        if s in name_lower:
            return 'system'
    return 'libraries'
...
async def scan_all_hosts(session: AsyncSession = Depends(get_db)):
    """Scan all hosts"""
    result = await session.execute(select(Host))
    hosts = result.scalars().all()
    ...
    for host in hosts:
        ...
        asyncio.create_task(_perform_scan(host.id, scan_history.id))
        scan_ids.append(scan_history.id)

    await session.commit()

    return {
        "message": f"Started scanning {len(hosts)} hosts",
        "scan_ids": scan_ids
    }


@router.get("/api/scan/{host_id}/status")
async def get_scan_status(host_id: int, session: AsyncSession = Depends(get_db)):
    """Get latest scan status for a host"""
    result = await session.execute(
        select(ScanHistory)
    ...
    if scan.scan_started:
        if scan.scan_completed:
            elapsed_seconds = int((scan.scan_completed - scan.scan_started).total_seconds())
        else:
            from datetime import datetime
            elapsed_seconds = int((datetime.utcnow() - scan.scan_started).total_seconds())
    ...
    return {
        ...
        "current_package": scan.current_package,
...
@router.get("/api/package-preview")
async def get_package_preview():
    """Get package counts by category for preview before scan"""
    try:
        scanner = PackageScanner()
        ...
        # Count by category
        counts = scanner.count_packages_by_category(packages)

        # Calculate estimated time
        # Real estimate:
        # - NVD API calls take ~0.6-2 seconds per package (with rate limiting)
        # - Fast mode with batch_size=10 processes 10 packages in ~8-12 seconds
        # - Normal mode processes sequentially: ~3-4 seconds per package
        total_packages = len(packages)

        # More realistic estimates
        fast_mode_seconds = (total_packages / 10) * 10  # 10 packages per ~10 seconds
        normal_mode_seconds = total_packages * 3.5  # ~3.5 seconds per package sequentially

        return {
            "total_packages": total_packages,
            "categories": counts,
            "estimated_time": {
                "fast_mode_seconds": int(fast_mode_seconds),
                "normal_mode_seconds": int(normal_mode_seconds),
                "fast_mode_minutes": round(fast_mode_seconds / 60, 1),
                "normal_mode_minutes": round(normal_mode_seconds / 60, 1),
                "fast_mode_hours": round(fast_mode_seconds / 3600, 1) if fast_mode_seconds > 3600 else None,
                "normal_mode_hours": round(normal_mode_seconds / 3600, 1) if normal_mode_seconds > 3600 else None
            }
        }
    except Exception as e:
...
async def get_patch_recommendation(package_id: int, session: AsyncSession = Depends(get_db)):
    ...
    # Get all findings for this package
    findings_result = await session.execute(
        select(Finding)
    ...
    for finding in findings:
        await session.refresh(finding, ["cve"])

    if not findings:
        return {"message": "No vulnerabilities found for this package"}

    # Generate patch recommendation
    advisor = PatchAdvisor()
    recommendation = advisor.generate_patch_recommendation(package, host, findings)
